...
var Analytics = class extends import_console.DataService {
  ...
  async getChatLunaUsageOverview() {
    ...
    const dayStart = import_koishi.Time.fromDateNumber(import_koishi.Time.getDateNumber());
    const weekStart = new Date(+end - import_koishi.Time.day * 7);
    const monthStart = new Date(+end - import_koishi.Time.day * 30);
    try {
  ...
  async getChatLunaModelUsage() {
    const end = /* @__PURE__ */ new Date();
    const ranges = {
      day: new Date(+end - import_koishi.Time.day),
      week: new Date(+end - import_koishi.Time.day * 7),
      month: new Date(+end - import_koishi.Time.day * 30)
    };
    const collect = /* @__PURE__ */ __name(async (start) => {
      try {
        ...
        return [...totals.entries()].map(([model, totalTokens]) => ({ model, totalTokens })).sort((a, b) => b.totalTokens - a.totalTokens).slice(0, 10);
      } catch (error) {
        logger.debug(error);
        return [];
      }
