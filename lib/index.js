  ...
  async getChatLunaSourceUsage() {
    ...
    const collect = /* @__PURE__ */ __name(async (start) => {
      try {
        ...
        return [...totals.entries()].map(([source, stats]) => ({ source, ...stats })).filter((item) => item.requests > 0).sort((a, b) => b.totalTokens - a.totalTokens || b.requests - a.requests);
      } catch (error) {
        logger.debug(error);
        return [];
      }
